-- 1 PRIVATE PROCEDURE IN PACKAGE
...
CREATE OR REPLACE TRIGGER customers_audit_trg1
    AFTER 
    UPDATE OR DELETE 
    ON customer_REFERENCE_INFO_2611
   FOR EACH ROW    
DECLARE
...
UPDATE CUSTOMER_REFERENCE_INFO_2611
SET RELATION='NEIGHBOUR'
WHERE CUSTOMER_ID='C-001';

DELETE FROM CUSTOMER_REFERENCE_INFO_2611 WHERE CUSTOMER_ID='C-001';
...
CREATE OR REPLACE TRIGGER customers_credit_trg1
...
DECLARE
    l_name customer_PERSONAL_INFO_2611.CUSTOMER_NAME%TYPE;
BEGIN
    -- IMPLICITLY RAISED
    -- get the customer 
    SELECT customer_PERSONAL_INFO_2611.CUSTOMER_NAME
    INTO l_name
...
-- 7 EXPLICIT RAISE PREDEFINDED EXCEPTION
DECLARE
    l_customer_id customer_PERSONAL_INFO_2611.customer_id%TYPE := &customer_id;
BEGIN
...
--declare
--    gender customer_personal_info.gender%type;
--    mail_id customer_personal_info.mail_id%type;
BEGIN
--    select gender,mail_id into gender,mail_id
--    from customer_personal_info where customer_id='C-001';
    delete CUSTOMER_PERSONAL_INFO_2611
            WHERE CUSTOMER_ID='C-001';
    UPDATE_MAIL;
    COMMIT;
--    select gender,mail_id into gender,mail_id
